...
export async function process(inscriptionId:string) {
  ...
  const binaryData = (base64Data) ? atob(base64Data) :  undefined; // Decode Base64 data to binary
...
// Decode WAV data
  const audioData = wav.decode(Buffer.from(arrayBuffer));
  ...
  const midiEvents = pitchDetection.map((pitch:any, index:number) => {
